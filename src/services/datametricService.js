...
export const getDataMetricReportByReportTypeAndDateRange = (data, successCallback) => {
    return async dispatch => {
        console.log(data);
        try {
            // const response = await fetch(`${GetDataMetricReportByReportTypeAndDateRange}?from=${data.fromdate.trim()}&to=${data.todate.trim()}&reportType=dashboard`);
            const response = await fetch(`${GetDataMetricReportByReportTypeAndDateRange}?from=2022-1-11&to=2022-2-11&reportType=dashboard`);
            if (response?.status === 200) {
            // if (response?.status === 200) {
